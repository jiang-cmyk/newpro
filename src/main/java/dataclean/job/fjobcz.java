...
import main.java.dataclean.mapper.fmappercz;
import main.java.dataclean.reduce.freducer;
import main.java.dataclean.reduce.freducercz;


public class fjobcz extends Configured implements Tool{
...
//		conf.set("mapred.textoutputformat.separator", ",");
		return ToolRunner.run(conf, new fjobcz(), args);
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(super.getConf(), "fcleancz");
		job.setJarByClass(fjobcz.class);
		//配置job任务对象
		job.setInputFormatClass(TextInputFormat.class);
		TextInputFormat.addInputPath(job, new Path(args[0]));
				//("file:///C:\\Users\\ABC\\Desktop\\2022.9月活跃度分析\\9月份活跃度数据\\主体登记/exportsql.20220925122644.gbk.csv"));
		job.setMapperClass(fmappercz.class);
		//设置K2，V2类型
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(Text.class);
				
		//reduce的处理方式
		job.setReducerClass(freducercz.class);
		...
		TextOutputFormat.setOutputPath(job,outputPath);
				//("file:///C:\\Users\\ABC\\Desktop\\Output/主体登记tep"));
